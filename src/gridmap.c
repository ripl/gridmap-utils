...
gridmap_lut_t *gridmap_lut_create_constant(int table_size, double max_dist, int inside_value, int outside_value)
{
    ...
    lut->max_dist_sq = bot_sq(max_dist);
                                        ...
                                        int *cost, int *restricted)
{
    ...
    *restricted = 0;
    ...
    for (int i = 0; i <= steps; i++, x+=dx, y+=dy) {
        ...
        cost_max        = bot_max(cost_max, (color&0xfe));
                                  ...
                                  double *cost, double *restricted)
{
    ...
    double dist = sqrt(bot_sq(x1-x0) + bot_sq(y1-y0));
...
void gridmap_draw_rectangle_max(gridmap_t *gm, double x0, double y0, double sizex, double sizey, uint8_t color)
{
    ...
    for (int y = ylo; y <= yhi; y++)
        for (int x = xlo; x <= xhi; x++)
            gm->data[y*gm->width + x] = bot_max(gm->data[y*gm->width+x], color);
}
    ...
    while (inpos < datalen) {
        ...
        while (run_length) {
            ...
            if (y < 0) {
                ...
                use_length = bot_min(run_length, xoff + width - x);
            } else if (x < 0) {
                // drop (up to) the beginning of the row.
                use_length = bot_min(run_length, -x);
            } else if (x >= gm->width) {
                // drop (up to) the rest of the row.
                use_length = bot_min(run_length, xoff + width - x);
            } else {
                ...
                use_length = bot_min(run_length, xoff + width - x);
                use_length = bot_min(use_length, gm->width - x);
...
int gridmap_decode_base(uint8_t *dest, int width, int height, uint8_t *src, int datalen)
{
    ...
    while (inpos < datalen) {
        ...
        while (run_length) {
            ...
            if (y < 0) {
                // drop (up to) the rest of the row.
                use_length = bot_min(run_length, width - x);
            } else if (x < 0) {
                // drop (up to) the beginning of the row.
                use_length = bot_min(run_length, -x);
            } else if (x >= width) {
                ...
                // data is *in bounds*. Copy up to the remainder of the row.
                use_length = bot_min(run_length, width - x);
                use_length = bot_min(use_length, width - x);
...
{
    double distance_sq = bot_sq(cx - gm->cx) + bot_sq(cy-gm->cy);
    if (distance_sq < bot_sq(max_distance))
        return 0;
                             ...
                             int xidx, int yidx, int xwidth, int ywidth)
{
    ...
    for (int x = 0; x < diam; x++) {
        for (int y = 0; y < diam; y++) {
            double dsq = bot_sq(y + .5 - diam/2.0) + bot_sq(x + .5 - diam/2.0);
            bitmask[x][y] = (dsq <= bot_sq(diam/2)) ? 1 : 0;
        }
    }
    ...
    */
    ...
    int ymin = bot_max(radius, yidx), ymax = bot_min(src->height - radius, yidx + ywidth);
    int xmin = bot_max(radius, xidx), xmax = bot_min(src->width - radius, xidx + xwidth);
...
/*
  ...
  ix0 = bot_clamp(ix0, 0, gm->width-1);
  ix1 = bot_clamp(ix1, 0, gm->width-1);
  iy0 = bot_clamp(iy0, 0, gm->height-1);
  iy1 = bot_clamp(iy1, 0, gm->height-1);
                        ...
                        const gridmap_lut_t *lut)
{
    ...
    double x_size = sqrt(bot_sq(xy0[0] - xy1[0]) + bot_sq(xy0[1] - xy1[1]));
    ...
    ix0 = bot_clamp(ix0, 0, gm->width-1);
    ix1 = bot_clamp(ix1, 0, gm->width-4);
    iy0 = bot_clamp(iy0, 0, gm->height-1);
    iy1 = bot_clamp(iy1, 0, gm->height-1);
                                       ...
                                       const gridmap_lut_t *lut)
{
    ...
    for (int i = 0; i < nx; i++) {
        ...
        ex[i] = dist2idx * bot_sq(fmax(0, fabs(dx) - x_size/2));
    ...
    for (int iy = iy0; iy <= iy1; iy++) {
        ...
        double ey = dist2idx * bot_sq(fmax(0, fabs(dy) - y_size/2));
        ...
        for (int ix = ix0; ix <= ix1; ix++) {
            ...
            if (idx < lut->size) {
                ...
                gm->data[iy*gm->width + ix] = bot_max(oldv, lut->table[idx]);
            }
        }
                                 ...
                                 const gridmap_lut_t *lut)
{
    ...
    for (int iy = iy0; iy <= iy1; iy++, y+= gm->meters_per_pixel) {
        ...
        for (int ix = ix0; ix <= ix1; ix+=4) {
            ...
            idx4 = _mm_min_ps(idx4, max_index4);
            float * idx;
            posix_memalign((void**)&idx, 16, 4*sizeof(float));
            //float idx[64];
            _mm_store_ps(idx, idx4);
            for (int i = 0; i < 4; i++) {
                int oldv = gm->data[iy*gm->width + ix + i];
...
//        return gridmap_render_rectangle_aligned(gm, cx, cy, x_size, y_size, lut);
    ...
    // offset so that we test the middle of every pixel
    //**** Not sure why this offset is applied ?? Seems to cause misalignment of the maps 
    ...
    for (int iy = iy0; iy <= iy1; iy++, y+= gm->meters_per_pixel) {
        ...
        for (int ix = ix0; ix <= ix1; ix+=4) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                int newv = bot_max(oldv, v) | ((oldv | v) & 1);
                gm->data[iy*gm->width + ix + i] = newv;
            }
...
int gridmap_rectangle_corner_min(gridmap_t *gm, double cx, double cy, double sx, double sy)
{
    ...
    return bot_min(bot_min(v00,v01), bot_min(v10, v11));
}
                          ...
                          uint8_t value)
{
    ...
    for (int iy = iy0; iy <= iy1; iy++, y+= gm->meters_per_pixel) {
        ...
        for (int ix = ix0; ix <= ix1; ix+=4) {
            ...
            float * c;
            posix_memalign((void**)&c, 16, 4*sizeof(float));
            _mm_store_ps(c, ca4);
...
//    printf("RECENTER %15f %5d %5d\n", distance_sq, dx, dy);
    ...
    if (dy >= 0) {
        ...
        for (int dest = gm->height-1; dest >= 0; dest--) {
            ...
            if (dx >= 0) {
                int sz = bot_max(0, gm->width - dx) * sizeof (uint16_t);
                memmove(&gm->data[dest*gm->width], 
                        &gm->data[src*gm->width + dx], sz);
                for (int i=0; i<gm->width - sz; i++) 
                    gm->data[dest*gm->width + sz + i] = fill;
...
*(dst->data+j*dst->width+i)=bot_max(*(src1->data+j*src1->width+i),*(src2->data+j*src2->width+i));
}
#else
...
dst_row[j] = bot_max (src_row1[j], src_row2[j]);
}
}
#endif
return 0;
...
*/
 ...
 /*
   ...
   gm->data[iy * gm->width + ix] = bot_max(orig_value, src_value * src_scale);
...
int gridmap_max_scale(gridmap_t *gm, const gridmap_t *src, double src_scale)
{
    ...
    for (int iy = iy0; iy <= iy1; iy++) {
        ...
        for (int ix = ix0; ix <= ix1; ix++) {
            ...
            gm->data[iy * gm->width + ix] = bot_max(orig_value, bot_min(255,src_value * src_scale));
...
int gridmap_max_scale_restricted(gridmap_t *gm, const gridmap_t *src, double src_scale)
{
    ...
    for (int iy = iy0; iy <= iy1; iy++) {
        ...
        for (int ix = ix0; ix <= ix1; ix++) {
            ...
            int new_value = bot_max(orig_value&0xfe, bot_min(254,src_value) & 0xfe);
...
//            listi->points[pidx].x = bot_clamp(ix, 0, gm->width-1);
//            listi->points[pidx].y = bot_clamp(iy, 0, gm->height-1);
        }
...
void gridmap_polygon_fill_min_restricted(gridmap_t *gm, polygon2d_t *polyd, int fill)
{
    ...
    for (int i = 0; i < interior->npoints; i++) {
        ...
        if (ix >= 0 && ix < gm->width && iy >= 0 && iy < gm->height) {
            int oldv = gm->data[iy*gm->width + ix];
            int newv = bot_min(fill, oldv) & 0xfe;
            newv |= (oldv & fill & 1);
