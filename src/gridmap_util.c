...
**/
gridmap_t *gridmap_util_import_tile(gridmap_t *gm, const gmlcm_gridmap_tile_t *tile, const double pos[3])
{
...
void gridmap_util_publish(const gridmap_t *gm, lcm_t *lcm, const char *channel, int64_t *generation, int64_t utime)
{
    int total_size = 0;
    int num_tiles = 0;

    gmlcm_gridmap_tile_t tile;
    memset(&tile, 0, sizeof(gmlcm_gridmap_tile_t));
    ...
    for (int x = 0; x < gm->width; x += GRIDMAP_TILE_PIXELS) {
        for (int y = 0; y < gm->height; y+= GRIDMAP_TILE_PIXELS) {
            ...
            tile.width = bot_min(gm->width - x, GRIDMAP_TILE_PIXELS);
            tile.height = bot_min(gm->height - y, GRIDMAP_TILE_PIXELS);
            ...
            gmlcm_gridmap_tile_t_publish(lcm, channel, &tile);
...
gridmap_util_import_tile_u16 (gridmap_u16_t *gm, 
        const gmlcm_gridmap_tile_t *tile, const double pos[3])
{
    if (gm == NULL || gm->meters_per_pixel != tile->meters_per_pixel) {
        double sizex = gm ? gm->sizex : 200;
        double sizey = gm ? gm->sizey : 200;
...
void 
gridmap_util_publish_u16 (const gridmap_u16_t *gm, lcm_t *lcm, 
